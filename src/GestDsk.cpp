...
// Color definitions (duplicated to avoid include issues)
...
string colorizeLocal(const string& text, const string& color) {
    extern bool useColors;
...
StAmsdos * CreeEnteteAmsdos( char * NomFic, unsigned short Longueur ) {
    ...
    char * p = NULL;
    do {
	p = strchr( NomReel, '/' ); //Under linux it's the / that needs to be removed ...
        if ( p )
            strcpy( NomReel, ++p );
    } while( p );
    p = strchr( NomReel, '.' );
    ...
    Entete.Length = 0; 	//Not filled by AMSDos !!
...
//
// Calculate and set AMSDOS checksum
//
...
//
// Perform AMSDOS header "cleanup":
// reset unused bytes to zero
//
...
//
// Search for smallest sector of a track
//
...
//
// Search for a free block and fill it
//
...
//
// Fill a "bitmap" to know where there are files on the floppy
// Also return the number of KB used on the floppy
//
...
//
// Check file existence, return file index if exists,
// -1 otherwise
//
int DSK::FileExist( char * Nom ) {
	...
	for ( i = 0; i < 64; i++ ) {
		...
		// Check if file has system attribute (like original iDSK)
		bool isSystemFile = (Dir->Ext[1] & 0x80) != 0;
		if (isSystemFile) {
			continue; // Skip system files - they should not be found in normal operations
		}
		
		// Check if file has read-only attribute (like original iDSK)
		bool isReadOnlyFile = (Dir->Ext[0] & 0x80) != 0;
		if (isReadOnlyFile) {
			continue; // Skip read-only files - they should not be found for deletion
		}
		
		for(int q=0;q<12;q++)
		    Dir->Nom[q]=Dir->Nom[q]&127; // Avoid missing hidden files
		if (  Dir->User != USER_DELETED 
			  && ! strncmp( Nom, ( char * )Dir->Nom, 11 ) // 11 = 8+3 because dot is removed
			  )
			return( i );
	}
	return( -1 );
...
// Copy a file to DSK
//
// size is determined by number of NbPages
// check why different from another DSK
int DSK::CopyFile( unsigned char * BufFile, char * NomFic, int TailleFic, int MaxBloc, int UserNumber, bool System_file, bool Read_only ) {
    int j, l, Bloc, PosFile, NbPages = 0, PosDir, TaillePage;
    FillBitmap();
    StDirEntry * DirLoc = GetNomDir( NomFic ); 	//Build entry to put in catalog
    for ( PosFile = 0; PosFile < TailleFic; ) { //For each file block
        PosDir = RechercheDirLibre(); 		//Find free entry in CAT
        if ( PosDir != -1 ) {
            DirLoc->User = UserNumber;			//Fill entry : User 0
            // http://www.cpm8680.com/cpmtools/cpm.htm
            if(System_file) DirLoc->Ext[1]|=0x80;
            if(Read_only) DirLoc->Ext[0]|=0x80;
            DirLoc->NumPage = ( unsigned char )NbPages++;	// Entry number in file
            TaillePage = (TailleFic - PosFile + 127) >> 7 ;	// Page size (rounded up)
            if ( TaillePage > 128 )				// If more than 16k need multiple pages
                TaillePage = 128;
			
            DirLoc->NbPages = ( unsigned char )TaillePage;
            l = ( DirLoc->NbPages + 7 ) >> 3; //Number of blocks=PageSize/8 rounded up
            memset( DirLoc->Blocks, 0, 16 );
            for ( j = 0; j < l; j++ ) { //For each page block
                Bloc = RechercheBlocLibre( MaxBloc );	//Put file on floppy
                if ( Bloc ) {
...
// Returns the new buffer size after conversion
//
unsigned long DSK::convertUnixToDos(unsigned char* buffer, unsigned long size) {
	static unsigned char tempBuffer[0x20000];
	unsigned long newSize = 0;
	bool hasConversion = false;
	
	for (unsigned long i = 0; i < size; i++) {
		if (buffer[i] == '\n' && (i == 0 || buffer[i-1] != '\r')) {
			// Found LF without preceding CR - convert to CRLF
			if (newSize < 0x20000 - 1) {
				tempBuffer[newSize++] = '\r';
				tempBuffer[newSize++] = '\n';
				hasConversion = true;
			} else {
				// Buffer overflow protection
				cerr << "Warning: Buffer overflow during DOS conversion, truncating file\n";
				break;
			}
		} else {
	...
	if (hasConversion) {
		cout << "Converted Unix line endings to DOS format (LF -> CRLF)\n";
	}
	...
	bool System_file, bool Read_only )
{
	...
	switch(TypeModeImport) {
		...
		case MODE_RAW :
			...
			// Convert Unix line endings (LF) to DOS line endings (CRLF) for raw mode too
			Lg = convertUnixToDos(Buff, Lg);
		break;
	}

	// Copy the file inside the DSK image.
	if (CopyFile(Buff, cFileName, Lg, 256, UserNumber, System_file, Read_only) != ERR_NO_ERR)
		ret = false;
	else
		ret = true;
...
std::string DSK::ReadDskDir( void ) {
	...
	int totalKB = 178; // Standard CPC 3.5" disk has 178 KB (178 blocks of 1KB each)
	int freeKB = totalKB - usedKB;
	
	// Header
	catalogue += "\n";
	catalogue += colorizeLocal("Filename     User  Load   Exec    Size  Attr", COLOR_BOLD COLOR_BRIGHT_BLUE) + "\n";
	catalogue += colorizeLocal("------------ ---- ------ ------ ------- ----", COLOR_BOLD COLOR_BRIGHT_BLUE) + "\n";
	...
	for ( int i = 0; i < 64; i++ ) {
		//
		// Display non-deleted files (including system files for better visibility)
		//
		if ( TabDir[ i ].User != USER_DELETED && ! TabDir[ i ].NumPage ) {
			...
			// Extract attributes before hiding attribute bits
			bool isReadOnly = (TabDir[ i ].Ext[0] & 0x80) != 0;
			bool isSystemFile = (TabDir[ i ].Ext[1] & 0x80) != 0;
			...
			// Format filename (pad to 12 characters)
			string filename = Nom;
			while (filename.length() < 12) {
				filename += " ";
			}
			catalogue += colorizeLocal(filename, COLOR_WHITE);
			
			// User number (pad to 4 characters)
			ostringstream userStr;
			userStr << (int)TabDir[i].User;
			string user = userStr.str();
			while (user.length() < 4) {
				user = " " + user;
			}
			catalogue += user;
			
			// Extract AMSDOS addresses from first block
			unsigned short loadAddr = 0;
			unsigned short execAddr = 0;
			if (TabDir[i].Blocks[0] > 0) {
				unsigned char * p = ReadBloc(TabDir[i].Blocks[0]);
				if (CheckAmsdos(p)) {
					loadAddr = p[0x15] + (p[0x16] << 8);
					execAddr = p[0x1a] + (p[0x1b] << 8);
				}
			}
			
			// Format Load address (exactly 6 characters)
			ostringstream loadStr;
			loadStr << "&" << hex << uppercase << setfill('0') << setw(4) << loadAddr;
			string loadFormatted = loadStr.str();
			// Pad to exactly 6 characters
			while (loadFormatted.length() < 6) {
				loadFormatted += " ";
			}
			catalogue += "  " + colorizeLocal(loadFormatted, COLOR_CYAN);
			
			// Format Execute address (exactly 6 characters)
			ostringstream execStr;
			execStr << "&" << hex << uppercase << setfill('0') << setw(4) << execAddr;
			string execFormatted = execStr.str();
			// Pad to exactly 6 characters
			while (execFormatted.length() < 6) {
				execFormatted += " ";
			}
			catalogue += " " + colorizeLocal(execFormatted, COLOR_CYAN);
			...
			int sizeKB = (t + 7) >> 3;  // Convert blocks to KB
			ostringstream sizeStr;
			sizeStr << sizeKB << " KB";
			string sizeFormatted = sizeStr.str();
			
			// Right-align size in 7-character field
			while (sizeFormatted.length() < 7) {
				sizeFormatted = " " + sizeFormatted;
			}
			catalogue += " " + colorizeLocal(sizeFormatted, COLOR_YELLOW);
			
			// Add attributes column
			string attributes = " ";
			if (isReadOnly) attributes += "R";
			if (isSystemFile) attributes += "S";
			// Pad to 4 characters
			while (attributes.length() < 4) {
				attributes += " ";
			}
			
			// Color attributes based on type
			string attrColor = COLOR_RESET;
			if (isReadOnly && isSystemFile) {
				attrColor = COLOR_BRIGHT_RED;  // Both R and S
			} else if (isSystemFile) {
				attrColor = COLOR_BRIGHT_MAGENTA;  // System file
			} else if (isReadOnly) {
				attrColor = COLOR_BRIGHT_YELLOW;   // Read-only
			}
			catalogue += colorizeLocal(attributes, attrColor);
	...
	catalogue += "\n";
	ostringstream freeStr;
	freeStr << freeKB << "K free";
	catalogue += colorizeLocal(freeStr.str(), COLOR_BRIGHT_GREEN) + "\n";
