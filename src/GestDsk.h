...
#pragma pack(1) //avoid padding of structures used in memcpy later

//
// AMSDOS entry structure
//
typedef struct
{
    unsigned char    UserNumber;             // 00 User
    unsigned char    FileName[15];           // 01-0F Nom + extension
    unsigned char    BlockNum;               // 10    Block number (floppy)
    unsigned char    LastBlock;              // 11    "Last block" flag (floppy)
    unsigned char    FileType;               // 12    File type
    unsigned short   Length;                 // 13-14 Length
    unsigned short   Adress;                 // 15-16 Address
    unsigned char    FirstBlock;             // 17    First file block flag (floppy)
    unsigned short   LogicalLength;          // 18-19 Logical length
    unsigned short   EntryAdress;            // 1A-1B Entry point
    unsigned char    Unused[0x24];
    unsigned short   RealLength;             // 40-42 Real length
    unsigned char    BigLength;              //       Real length (3 bytes)
...
class DSK
{
	...
	unsigned long convertUnixToDos( unsigned char* buffer, unsigned long size );
...
public:
	...
	int CopyFile( unsigned char * BufFile, char * NomFic, int TailleFic, int MaxBloc, int, bool,bool );
